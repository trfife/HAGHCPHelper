...
class GHCPConversationConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_github(
        ...
        """Configure GitHub Models backend."""
        errors: dict[str, str] = {}
...
class GHCPOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        if backend == BACKEND_GITHUB:
            schema_dict[vol.Required(CONF_GITHUB_TOKEN, default=entry_data.get(CONF_GITHUB_TOKEN, ""))] = TextSelector(
                TextSelectorConfig(type=TextSelectorType.PASSWORD)
            )
            schema_dict[vol.Optional(CONF_MODEL, default=entry_data.get(CONF_MODEL, DEFAULT_MODEL))] = SelectSelector(
                SelectSelectorConfig(
                    options=MODEL_OPTIONS,
                    custom_value=True,
                    mode=SelectSelectorMode.DROPDOWN,
            ...
            schema_dict[vol.Required(CONF_AZURE_ENDPOINT, default=entry_data.get(CONF_AZURE_ENDPOINT, ""))] = TextSelector(
                TextSelectorConfig(type=TextSelectorType.URL)
            )
            schema_dict[vol.Required(CONF_AZURE_API_KEY, default=entry_data.get(CONF_AZURE_API_KEY, ""))] = TextSelector(
                TextSelectorConfig(type=TextSelectorType.PASSWORD)
            )
            schema_dict[vol.Required(CONF_MODEL, default=entry_data.get(CONF_MODEL, ""))] = TextSelector(
                TextSelectorConfig(type=TextSelectorType.TEXT)
            )
...
class GHCPConversationSubentryFlow:
    ...
    async def async_step_user(
        ...
        hass_apis: list[SelectSelectorConfig] = [
            SelectSelectorConfig(
                options=[
